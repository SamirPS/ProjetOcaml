...
let rec transitionexist nomdunoeud listtransition =
  match listtransition with 
  | [] -> false
  | Edge(a,b,c,d)::q when ( (a = nomdunoeud) || (b = nomdunoeud)) -> true
  | _::q -> transitionexist nomdunoeud q;;

let rec is_complete listenoeud listtransition = 
    match listenoeud with 
    | [] -> true
    | Noeud(a,b,c,d)::q when (transitionexist a listtransition = false) -> false
    | _::q -> is_complete q listtransition;; 